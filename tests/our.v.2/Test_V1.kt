...
class MainActivityUITest {
    ...
    fun testTitleText_isDisplayed() {
        onView(withId(R.id.title))
            ...
            .check(matches(withText("GUI Test Interface V2")))
    }
    ...
    // 4️⃣ Test RecyclerView Displays Items
    @Test
    fun testRecyclerViewDisplaysItems() {
        onView(withText("Gigio")).check(matches(isDisplayed()))
        onView(withText("Item 5")).check(matches(isDisplayed()))
    ...
    fun testImageView_isDisplayed() {
        onView(withId(R.id.sampleImageV2))
            .check(matches(isDisplayed()))
    }
}